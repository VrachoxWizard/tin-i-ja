...
import { brokerUploadDealRoomFileAction } from "@/app/actions/broker";
...
export function BrokerDealRoomUploadForm({
  listingId,
}: {
  listingId: string;
}) {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result = await brokerUploadDealRoomFileAction(listingId, payload);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="broker-deal-room-file-input">Datoteka</Label>
        <Input
          id="broker-deal-room-file-input"
          type="file"
