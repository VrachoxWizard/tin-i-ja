      ...
      <Card className="border-df-trust-blue/20 bg-white/5 dark:bg-slate-900/40 backdrop-blur-xl shadow-glass max-w-2xl mx-auto text-center py-12 rounded-2xl relative overflow-hidden">
        ...
        <CardContent className="flex flex-col items-center justify-center space-y-6 relative z-10">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white font-jakarta tracking-tight">Profil Spremljen!</h2>
          ...
          <Button variant="default" className="mt-6 bg-df-trust-blue hover:bg-df-trust-blue/90 text-white font-jakarta rounded-xl shadow-lg hover:shadow-xl transition-all h-12 px-8" onClick={() => window.location.href = '/listings'}>
  ...
  return (
    <Card className="border-slate-200/50 dark:border-slate-800/50 bg-white/50 dark:bg-slate-900/40 backdrop-blur-xl shadow-glass relative overflow-hidden rounded-2xl font-inter">
      ...
      <CardHeader className="pt-10 px-8">
        ...
        <CardDescription className="text-base text-slate-500 dark:text-slate-400 font-inter mt-1">Definirajte svoje investicijske parametre.</CardDescription>
      </CardHeader>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }} className="space-y-6">
                <div className="space-y-3">
                  <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Tip Investitora</Label>
                  <Select value={formData.buyer_type} onValueChange={(val) => handleSelectChange('buyer_type', val)} required>
                    <SelectTrigger className="h-12 bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-df-trust-blue/50 transition-all font-inter"><SelectValue placeholder="Odaberite tip" /></SelectTrigger>
                    <SelectContent className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-slate-200 dark:border-slate-700 rounded-xl font-inter">
                      <SelectItem value="individual" className="focus:bg-slate-100 dark:focus:bg-slate-800 cursor-pointer">Fizička osoba / Individualni Poduzetnik</SelectItem>
                      <SelectItem value="strategic" className="focus:bg-slate-100 dark:focus:bg-slate-800 cursor-pointer">Strateški Investitor (Tvrtka / Korporacija)</SelectItem>
                      <SelectItem value="financial" className="focus:bg-slate-100 dark:focus:bg-slate-800 cursor-pointer">Financijski Investitor (PE / VC Fond)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-3">
                    <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Minimalna Investicija (EUR)</Label>
                    <Input type="number" name="investment_min" value={formData.investment_min} onChange={handleChange} required placeholder="Npr. 50000" className="h-12 bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-df-trust-blue/50 transition-all font-inter" />
                  </div>
                  <div className="space-y-3">
                    <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Maksimalna Investicija (EUR)</Label>
                    <Input type="number" name="investment_max" value={formData.investment_max} onChange={handleChange} required placeholder="Npr. 2000000" className="h-12 bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-df-trust-blue/50 transition-all font-inter" />
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }} className="space-y-6">
                <div className="space-y-3">
                  <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Primarni Sektor Interesa</Label>
                  <Select value={formData.target_industries} onValueChange={(val) => handleSelectChange('target_industries', val)} required>
                    <SelectTrigger className="h-12 bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-df-trust-blue/50 transition-all font-inter"><SelectValue placeholder="Odaberite industriju" /></SelectTrigger>
                    <SelectContent className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-slate-200 dark:border-slate-700 rounded-xl font-inter">
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Primarna Regija</Label>
                  <Select value={formData.target_regions} onValueChange={(val) => handleSelectChange('target_regions', val)} required>
                    <SelectTrigger className="h-12 bg-white/70 dark:bg-slate-900/60 border-slate-200 dark:border-slate-700 rounded-xl focus:ring-df-trust-blue/50 transition-all font-inter"><SelectValue placeholder="Odaberite regiju" /></SelectTrigger>
                    <SelectContent className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-slate-200 dark:border-slate-700 rounded-xl font-inter">
            ...
            {step === 3 && (
              <motion.div key="step3" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }} className="space-y-6">
                <div className="space-y-3">
                  <Label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Investicijska Teza i Motivacija</Label>
                  <Textarea 
                  ...
                  />
                  <p className="text-xs text-slate-500 font-medium">Ova informacija pomaže prodavateljima da vas procijene prije odobrenja NDA-a.</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
