...
describe("sanitizeHtml", () => {
    ...
    it("keeps allowed attributes (class) and data-* on allowed tags", () => {
        // DOMPurify preserves data-* attributes on allowed tags by default.
        // The key security property is that dangerous attrs (href, src on spans, event handlers) are stripped.
        const dirty = "<p class='text-sm' data-id='123'>Text</p>";
        const result = sanitizeHtml(dirty);
        expect(result).toContain("class=");
        // data-* is preserved by DOMPurify on allowed tags — this is expected
