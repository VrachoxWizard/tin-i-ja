  ...
  return (
    <div className="flex min-h-screen flex-col bg-background">
      <main className="flex-1 w-full bg-slate-50 dark:bg-slate-950 py-12 relative overflow-hidden">
        ...
        <div className="absolute top-[-10%] right-[-10%] w-125 h-125 bg-df-gold/10 rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute bottom-[-10%] left-[-10%] w-125 h-125 bg-df-trust-blue/10 rounded-full blur-[120px] pointer-events-none" />

        <div className="container mx-auto px-4 max-w-6xl relative z-10">
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white font-jakarta tracking-tight mb-8">
            Nadzorna Ploča{" "}
            <span className="text-slate-500 dark:text-slate-400 font-dm-sans text-xl ml-2 font-normal">
              (Prodavatelj)
            </span>
          </h1>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <GlowCard className="border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl shadow-glass rounded-2xl overflow-hidden hover:shadow-lg transition-all group">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-linear-to-br from-indigo-500/20 to-blue-500/10 flex items-center justify-center group-hover:from-indigo-500/30 group-hover:to-blue-500/20 transition-colors">
              ...
              <CardContent>
                <div className="text-4xl font-bold text-slate-900 dark:text-white font-heading tracking-tight">
                  —
                </div>
            ...
            <GlowCard className="border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl shadow-glass rounded-2xl overflow-hidden hover:shadow-lg transition-all group">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-linear-to-br from-emerald-500/20 to-green-500/10 flex items-center justify-center group-hover:from-emerald-500/30 group-hover:to-green-500/20 transition-colors">
            ...
            <GlowCard className="border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl shadow-glass rounded-2xl overflow-hidden hover:shadow-lg transition-all ring-1 ring-df-trust-blue/10 group">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs font-bold text-df-trust-blue uppercase tracking-widest flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-linear-to-br from-gold/20 to-amber-500/10 flex items-center justify-center group-hover:from-gold/30 group-hover:to-amber-500/20 transition-colors">
              ...
              <CardContent>
                <div className="text-4xl font-bold text-gold font-heading tracking-tight">
                  {ndaList.length}
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {ndaList.length === 0
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <CardHeader className="bg-white/40 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800/60 px-6 py-5">
                  <div className="flex justify-between items-center">
                    ...
                    {listing ? (
                      ...
                      <Badge className="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 border-none">
                        Nema Oglasa
                      </Badge>
                    )}
                  </div>
                  <CardDescription className="text-slate-500 dark:text-slate-400 font-inter mt-1">
                  ...
                  {listing ? (
                    <>
                      ...
                      <Link href="/listings">
                        <Button
                          variant="outline"
                          className="font-jakarta rounded-xl border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 h-10 px-5"
                        >
            ...
            <div className="space-y-6">
              <GlowCard className="border-slate-200/50 dark:border-slate-800/50 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl shadow-glass rounded-2xl overflow-hidden">
                <CardHeader className="bg-white/40 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800/60 px-6 py-5">
                  <CardTitle className="text-xl text-slate-900 dark:text-white font-jakarta">
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  {ndaList.map((req: any) => (
                    <div
                      ...
                      className={`p-5 border rounded-xl bg-white/80 dark:bg-slate-900/80 shadow-sm hover:shadow-md transition-all flex flex-col gap-4 ${req.status === "pending" ? "border-l-4 border-l-gold border-slate-200/60 dark:border-l-gold dark:border-slate-700/60" : "border-slate-200/60 dark:border-slate-700/60"}`}
                    >
                      <div className="flex justify-between items-start gap-4">
                        <div>
                          <p className="font-bold text-sm text-slate-900 dark:text-white font-jakarta mb-1">
                            {req.users?.full_name || "Skriveni Investitor"}
                          </p>
                          <p className="text-xs text-slate-500 dark:text-slate-400 font-inter flex items-center">
                        ...
                        {req.status === "pending" ? (
                          <Badge
                            variant="outline"
                            className="bg-amber-50/80 text-amber-700 border-amber-200/60 font-medium whitespace-nowrap"
                          >
                            Na čekanju
                          </Badge>
                        ) : req.status === "signed" ? (
                          <Badge
                            variant="outline"
                            className="bg-green-50/80 text-green-700 border-green-200/60 font-medium whitespace-nowrap"
                          >
                            Odobreno
                          </Badge>
                        ) : (
                          <Badge
                            variant="outline"
                            className="bg-red-50/80 text-red-700 border-red-200/60 font-medium whitespace-nowrap"
                          >
                            Odbijeno
                          </Badge>
                        )}
